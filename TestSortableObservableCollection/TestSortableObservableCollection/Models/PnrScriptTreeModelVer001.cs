    ...
    public class PnrScriptTreeModelVer001 : IPnrScriptTreeModel
    {
        public string LoadTree(PnrScriptTreeViewModel vm)
        {
            ...
            if (vm != null && vm.Root != null)
            {
                ...
                if (vm.Root.Count == 0)
                {
                    IPnrScriptSubgroupViewModel rootItem = new PnrScriptSubgroupViewModel(Constants.WindowMode.None, null, "Root", null, null);
            ...
            try
            {
                    ...
                    else if (inner.NodeType == XmlNodeType.Text)
                    {
                        switch (elementName)
                        {
                            ...
                            case "GUID":
                                ...
                                if (cmd != null)
                                {
                                    IGDSCommandViewModel newItem = new GDSCommandViewModel(cmd.CurrentWindowMode, cmd.Parent, cmd.Description, cmd.CommandLines, cmd.Guid, null, null);
                                    gdsCmds.Add(newItem);
                                }
                                break;
                        }
                ...
                if (nodeParsedCorrectly)
                {
                    ...
                    if (classType.Contains("PnrScriptSubgroupViewModel"))
                    {
                        if (intLevel == 0)
                        {
                            newItem = new PnrScriptSubgroupViewModel(Constants.WindowMode.None, null, description, null, null);
                            vm.Root.Add(newItem);
                        }
                        else
                        {
                            ...
                            if (parent != null)
                            {
                                newItem = new PnrScriptSubgroupViewModel(Constants.WindowMode.None, parent, description, null, null);
